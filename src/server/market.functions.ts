...
export const analyzeMarket = createServerFn({ method: "POST" })
  .inputValidator((data: AnalyzeInput) => {
    if (!data?.idea || typeof data.idea !== "string" || data.idea.trim().length < 5) {
      throw new Error("Descreva a ideia com pelo menos 5 caracteres.");
    }
...
Gere uma análise completa de mercado com TAM, SAM, SOM, score de oportunidade, insights estratégicos, sugestões de produtos, nichos ocultos e veredito final. Use a função return_market_analysis.`;
    ...
    if (res.status === 402) {
      throw new Error("PAYMENT_REQUIRED:Créditos de IA esgotados. Adicione créditos em Settings > Workspace > Usage.");
    }
    if (!res.ok) {
...
Use a função return_niche_opportunities.`,
    ...
    if (res.status === 429) throw new Error("RATE_LIMIT:Aguarde alguns segundos.");
    if (res.status === 402) throw new Error("PAYMENT_REQUIRED:Créditos esgotados.");
    if (!res.ok) throw new Error("Falha ao expandir nichos.");

    const json = await res.json();
    const args = json.choices?.[0]?.message?.tool_calls?.[0]?.function?.arguments;
    if (!args) throw new Error("Resposta inválida.");
    const parsed = JSON.parse(args);

    // Persistir no Lovable Cloud se houver analysisId e nichos
    if (data.analysisId && parsed.niches?.length > 0) {
      const { getSupabase } = await import("./leads-core");
      const supabase = getSupabase();
      
      const opportunities = parsed.niches.map((n: any) => ({
      ...
      await supabase.from("market_niche_opportunities").insert(opportunities);
    }
