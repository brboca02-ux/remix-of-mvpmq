...
BEGIN
    WITH filtered_leads AS (
        SELECT 
            COALESCE(NULLIF(porte, ''), 'Não informado') as porte,
    ...
    stats AS (
        SELECT 
            ...
            COALESCE(jsonb_object_agg(porte, count_porte) FILTER (WHERE porte IS NOT NULL), '{}'::jsonb) as distribuicao_porte
        FROM (
    ...
    geo_stats AS (
        SELECT COALESCE(jsonb_object_agg(uf, count_uf) FILTER (WHERE uf IS NOT NULL), '{}'::jsonb) as distribuicao_estado
        FROM (
